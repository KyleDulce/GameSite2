...
public abstract class Room {
    ...
    /**
     * This method is for common logic among games for handling GameData such as broadcasting chat
     * messages. A separate protected method, processGameDataForGame is called to handle game
     * specific GameData processing.
     *
    ...
    protected void selectNewRandomHost() {
        ...
        messengerService.sendMessageToUser(
                ...
                new BlankGameData(getRoomId(), GameDataType.CHANGE_HOST)
    ...
    private boolean processPlayerKickRequest(User user, KickPlayerData data) {
        ...
        if (GamesiteUtils.isNotBlank(data.player.getSocketId())) {
            messengerService.sendMessageToUser(
                    data.player,
                    SocketMessengerService.SocketDestinations.GAMEDATA,
                    new BlankGameData(getRoomId(), GameDataType.FORCE_KICK)
                            .parseObjectToDataMessage());
        }
        return true;
    }
    ...
    protected void sendDataResponseUpdate() {
        if (GamesiteUtils.isNotBlank(host.getSocketId())) {
            messengerService.sendMessageToUser(
                    host,
                    SocketMessengerService.SocketDestinations.GAMEDATA,
                    new SettingsDataResponse(getRoomId(), usersJoinedList)
                            .parseObjectToDataMessage());
        }
    }
